...
setwd("D:\\online course\\datascience course\\vit friends\\exams\\Lesson 8 Practical Machine Learning\\4 week\\")
file.training <- "pml-training.csv"
file.testing <- "pml-testing.csv"

data.training = read.csv(file.training, na.strings=c("", "NA", "NULL"))
data.testing = read.csv(file.testing, na.strings=c("", "NA", "NULL"))
...
remove the row having NA values
```{r}
training.cleaned <- data.training[ , colSums(is.na(data.training)) == 0]
#testing.cleaned <- data.testing[ , colSums(is.na(data.testing)) == 0]
```

####Remove unrelevant variables
```{r}
remove = c('X', 'user_name', 'raw_timestamp_part_1', 'raw_timestamp_part_2', 'cvtd_timestamp', 'new_window', 'num_window')
training.rmVariable <- training.cleaned[, -which(names(training.cleaned) %in% remove)]
dim(training.rmVariable)
#testing.rmVariable <- testing.cleaned[, -which(names(testing.cleaned) %in% remove)]
#dim(testing.rmVariable)
```
## Low variance check
find the variables that have extremely low variance (this method is useful nearZeroVar() )
```{r}
library(caret)
training.zeroVar= nearZeroVar(training.rmVariable[sapply(training.rmVariable, is.numeric)], saveMetrics = TRUE)
training.nonzerovar = training.rmVariable[,training.zeroVar[, 'nzv']==0]
dim(training.nonzerovar)
```

## Remove Correlated variables
Now, remove highly correlated variables 90% (using for example findCorrelation() )

#### here we can only use numeric variabls
```{r}
correlate.Matrix <- cor(na.omit(training.nonzerovar[sapply(training.nonzerovar, is.numeric)]))
dim(correlate.Matrix)
```
here we have total 52 variables
```{r}
correlateDF <- expand.grid(row = 1:52, col = 1:52)
correlateDF$correlation <- as.vector(correlate.Matrix)
levelplot(correlation ~ row+ col, correlateDF)

```

##Remove varible having high correlation
```{r}
correlate.remove = findCorrelation(correlate.Matrix, cutoff = .90, verbose = TRUE)
training.decor = training.nonzerovar[,-correlate.remove]
dim(training.decor)
```
...
##Analysis Regression Tree
here fit a tree to these data, and summarize and plot it. 
First, we use the 'tree' package, since it is much faster than 'caret' package.
```{r}
library(tree)
set.seed(10000)
...
```{r}
answers <- predict(rf.training, data.testing)
answers
```
